...
for CHART_DIR in ${CHART_DIRS}; do
  ...
  helm dep up "${CHART_DIR}" && helm template --values "${CHART_DIR}"/values.yaml "${CHART_DIR}" | ./kubeval --strict --ignore-missing-schemas --kubernetes-version "${KUBERNETES_VERSION#v}" --schema-location "${SCHEMA_LOCATION}"
done